function executeRR(patientList, doctorsCount, quantum) {
  ...
  registerEvent(eventsPanel, `<i class="fa fa-play" style="color: #23501bff"></i> Início da simulação Round-Robin com ${doctorsCount} médico(s).`);
  ...
  while (
    ...
    while (
      ...
      registerEvent(eventsPanel, `<i class="fa fa-user-plus" style="color: #23501bff"></i> ${newPatient.nome} chegou no tempo ${newPatient.chegada}.`);
    }
    ...
    for (let i = 0; i < doctorsCount; i++) {
      ...
      if (doctor.currentPatient === null && waitingQueue.length > 0) {
        ...
        registerEvent(eventsPanel, `<i class="fa fa-user-doctor" style="color: #23501bff"></i> ${nextPatient.nome} iniciou com Médico ${i + 1} no tempo ${currentTime}.`);
      }
    }
      ...
      if (current !== null) {
        ...
        if (current.remainingDuration === 0 || doctor.timeSlice === quantum) {
          ...
          if (current.remainingDuration === 0) {
            ...
            registerEvent(eventsPanel, `<i class="fa fa-check" style="color: #23501bff"></i> ${current.nome} finalizou com Médico ${i + 1} no tempo ${endTime}.`);
          } else {
            current.lastEndTime = endTime;
            waitingQueue.push(current);
            registerEvent(eventsPanel, `<i class="fa fa-undo" style="color: #c47e1c"></i> ${current.nome} foi interrompido após ${quantum} unidades no Médico ${i + 1}.`);
          }
  ...
  const utilizationRate = ((totalOccupation / (currentTime * doctorsCount)) * 100).toFixed(2);
  ...
  registerEvent(eventsPanel, `<i class="fa fa-check" style="color: #23501bff"></i> Simulação finalizada no tempo ${currentTime}.`);
}
